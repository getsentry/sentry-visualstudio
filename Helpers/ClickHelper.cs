...
namespace VSSentry.Helpers
{
    public static class ClickHelper
    {
        ...
        public static bool HandleFileLinkClicked(string[] input, int? lineNumber)
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    try
                    {
                        (EnvDteHelper.Dte.ActiveDocument?.Selection as TextSelection)?.GotoLine(lineNumber ?? int.Parse(input[1]));
                    }
                    catch (Exception)
                    {
        ...
        public static bool HandleMemberLinkClicked(string[] input, int? lineNumber)
        {
            try
            {
                ...
                (EnvDteHelper.Dte.ActiveDocument.Selection as TextSelection)?.GotoLine(lineNumber ?? location.GetLineSpan().StartLinePosition.Line + 1);
